---
description: "Mesen debugger execution tools: initialization, playback control, stepping, breakpoints, CPU/PPU state inspection, call stacks, expression evaluation. Use when debugging ROM code, setting breakpoints, stepping through instructions, or inspecting registers in any supported system (NES, SNES, Game Boy, GBA, PC Engine, SMS, WonderSwan)."
---
...
|------|-----------|-------------|
| `mesen_init_debugger()` | -- | Initialize the debugger. Must be called before using any debug tools. |

**Notes:**
- This MUST be called before using any other tool in this skill. Without initialization, all debug tools will fail.
...
| `mesen_resume_execution()` | -- | Resume execution after a breakpoint or step |
| `mesen_is_execution_paused()` | -- | Check if execution is paused. Returns `{paused: bool, running: bool}` |
...
- `mesen_is_execution_paused()` is useful for polling state after asynchronous operations.

## Stepping (1 tool)

| Tool | Parameters | Description |
|------|-----------|-------------|
| `mesen_step(cpuType, count?, stepType?)` | cpuType (required), count=1, stepType=Step | Step the CPU. Returns CPU state after stepping. |
...
| Tool | Parameters | Description |
|------|-----------|-------------|
| `mesen_breakpoint(action, ...)` | action: set\|remove\|remove_all\|list | Manage breakpoints and watchpoints |
...
| Tool | Parameters | Description |
|------|-----------|-------------|
| `mesen_get_state(component, cpuType)` | component: cpu\|ppu, cpuType required | Get CPU or PPU state (registers, flags, PC) |
| `mesen_set_program_counter(cpuType, address)` | cpuType, address (dec or 0x/$ hex) | Set the program counter to a specific address |
| `mesen_get_callstack(cpuType)` | cpuType required | Get current call stack (subroutine return addresses) |
| `mesen_evaluate_expression(expression, cpuType)` | expression string, cpuType | Evaluate debugger expression. Returns value in decimal and hex. |

**Expression examples:** `"A + X"`, `"$4016"`, `"[0x2000]"`, `"Y * 2 + $10"`

**Notes:**
- `mesen_get_state(component="cpu", ...)` returns all CPU registers, flags, and the program counter. Use `component="ppu"` for PPU-specific state (scanline, cycle, frame count, etc.).
...
- `mesen_init_debugger()` MUST be called before using any of these tools.
